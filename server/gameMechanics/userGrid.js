...
class UserGrid {
    ...
    constructor(difficulty = "beginner", answerGrid = null, userGrid = null) {
        this.difficulty = difficulty
        this.size = 9;
        this.blanks = 81;
        this.mines = 10;
        this.answerGrid = answerGrid && userGrid ? answerGrid:  new Array(this.size).fill(0).map(() => new Array(this.size).fill(0)) 
        answerGrid && userGrid ? undefined: this.generateMines()
        answerGrid && userGrid ? undefined: this.fillMinecount()
        this.userGrid = answerGrid && userGrid ? userGrid: new Array(this.size).fill(0).map(() => new Array(this.size).fill(BLANK)) 
    }
    ...
    click(row, col) {
        ...
        console.log(answerCellValue)
        switch (answerCellValue) {
            case MINE:
                // this.setUserGridCell(row, col, MINE)
                this.setUserGrid(this.answerGrid)
                break;
            case 0:
        ...
        while (queue.length > 0) {
            ...
            if (minecount === 0) {
                for (let p = 0; p < OFFSET.length; p++) { //process neighbors of cur
                    ...
                    // this.setUserGridCell(row, col, answerGrid[row][col])
    ...
    outOfBounds(row, col) {
        console.log(this)
        return row < 0 || col < 0 || row >= this.getAnswerGrid().length || col >= this.getAnswerGrid()[0].length;
    }
